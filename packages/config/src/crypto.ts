/* Browser-safe synchronous primitives used by the shared config barrel.
 * Node's `node:crypto` cannot be imported by the Next.js client bundle. */
...
/** Synchronous SHA-256 for deterministic, non-secret content identifiers. */
export function sha256(value: string): string {
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + sum1 + choice + roundConstants[index]! + schedule[index]!) >>> 0;
