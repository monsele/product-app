...
function propsFor(
  ...
  return {
    ...
    captions: timeline.map((segment, index) => ({
      ...
      text: supportedLesson.scenes[index]!.narration,
    })),
    creativeDesign: creativeDesignManifestSchema.parse({
      ...
      selections: Object.fromEntries(
        Object.entries(selections).map(([sceneId, selection]) => [
          sceneId,
          {
            ...selection,
            treatmentId: `${packId}.${supportedLesson.scenes.find((scene) => scene.id === sceneId)!.template}.${variant}`,
          },
        ]),
      ),
...
describe("ST-101 creative design production rendering", () => {
    ...
    for (const packId of [
      ...
      for (const variant of ["primary", "alternate"] as const) {
        ...
        for (const segment of timeline) {
          ...
          hashes.add(
            createHash("sha256").update(rendered.buffer!).digest("hex"),
          );
        }
      }
