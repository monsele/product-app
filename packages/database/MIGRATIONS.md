...
- Applied migrations are not rolled back in place unless a reviewed operation can preserve the exact journal state and proves that no newer migration is applied. Otherwise, include compatibility notes and use a reviewed forward migration or restore plan. `0000_dark_phalanx` follows the compatibility-note path.
