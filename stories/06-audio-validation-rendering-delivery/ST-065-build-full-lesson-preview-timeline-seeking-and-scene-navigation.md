...
### Playback repetition repair — 2026-09-12

- **Problem and evidence:** The owner's existing seven-scene lesson reproduced three unsolicited backward audio seeks of approximately 0.66 seconds during 17 seconds of ordinary hydrated Player playback. Frame-counter updates reparsed the input and recreated `Player.inputProps`. In installed Remotion 4.0.507, input-prop identity changes recreate composition metadata and restart the playback effect's time anchor. Audio then seeks backward to the lagging timeline, audibly replaying speech. This is distinct from repeated words inside a synthesized file.
- **Implementation plan and changes:** Memoize validated input and composition props in both full-lesson and single-scene players; stabilize the full-lesson audio-error callback; opt narration into media-buffering pauses. Keep real input/quality changes responsive and preserve existing error recovery. Add hydrated Chromium regression tests using local PCM audio and simulated per-frame visual work, covering clock progression, absence of unsolicited backward seeks, pause/resume, replay, scene navigation, and captions. No synchronization tolerance was widened and no audio gate was introduced.
- **Files:** `packages/scene-library/src/full-lesson.tsx`, `scene-preview.tsx`, `preview-playback.fixture.tsx`, `preview-playback.test.ts`, this record, and `STORY_INDEX.md`.
- **Across-pipeline investigation:** All seven private WAV downloads matched their stored checksums, declared data lengths, and 24 kHz format, with no clipped samples or identical repeated 100 ms blocks. Saved speaking rate was 1, so PCM speed conversion was bypassed. TTS sends whole scenes in non-streaming requests and does not concatenate retry responses. All seven TTS jobs succeeded on their first job attempt (this does not prove absence of internal HTTP retries). Four alignment failures already fell back to estimated captions without blocking audio. Scene durations already follow measured audio under ADR-004. A local, unprompted Whisper base.en diagnostic did not find widespread adjacent repetition; one suspected final-word repeat in scene 2 had only 0.065 probability in a second decode, so synthesis defects are not conclusively ruled out. No raw script, transcript, provider payload, or signed URL was logged by these diagnostics.
- **Validation:** Before/after instrumented Chromium playback of the same stored lesson/audio: three backward seeks before, zero after in both normal and deliberately stalled playback. New hydrated playback tests pass for both players. The 14 existing scene/full-preview unit tests and all 26 TTS/alignment/scene-job tests pass, including the non-blocking alignment fallback. Shared workspace builds, scene-library lint/typecheck, web typecheck, and preview/render frame parity were checked. Retained the full-lesson file's existing CRLF convention; whitespace check uses `git -c core.whitespace=cr-at-eol diff --check`.
- **Migrations / public contracts:** None. No stored audio, narration, captions, immutable lesson version, provider setting, or paid usage was changed. No paid generation/transcription calls were made. Private diagnostic audio and the isolated local transcription environment were moved outside the worktree after inspection.
- **Decisions / risks / deviations:** Maintenance of the completed preview story on the owner's existing branch, not a pipeline redesign. The run-app workflow informed verification with the real shared Player and stored lesson assets, plus rebuilding shared packages consumed from `dist`; the authenticated Next.js route was not exercised. Browser scheduling differs across devices; this fixes a reproduced playback cause, not a guarantee against every possible speech-model defect. The low-confidence scene-2 raw-audio flag remains a listening follow-up, not a reason to fail a job or regenerate all scenes.
